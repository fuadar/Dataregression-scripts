...
# train test split 
sample = sample.split(tahoe$readmit30, SplitRatio = 0.5)
#sample = sample.split(tahoe, SplitRatio = 0.15)# seelect a random sample of 80%
...
#nn_model <- knn(train = X_train[,1:3], test=X_test[,1:3], cl = y_train, k=3)
#summary(nn_model)
#confusionMatrix(nn_model, y_test)# accuracy of prediction on test data 
# Accuracy 
# 0.67
nn_model <- knn(train = X_train, test=X_test, cl = y_train, k=5)
summary(nn_model)
confusionMatrix(nn_model, as.factor(y_test))
...
for(i in 1:20) { # we will loop through K= 1 to 20
  knn_model <- knn(train = X_train[1:5], test=X_test[1:5], cl = y_train, k = i)
...
# K = 16
# Evaluate model at K =11
accuracy.df[16,] # read 11th row
# accuracy 0.7180984
...
for(i in 1:20) {
  ...
  accuracy.df[i, 2] <- confusionMatrix(knn_model, y_test)
}
